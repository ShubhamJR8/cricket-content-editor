...
const WorkspacePanel = () => {
  const { 
    ...
    addPlayer, handleCopyUnassigned, copied, downloadAllFiles
  } = useVideoEditor();
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            {downloadUrls.length > 0 && (
              ...
              >
                <Download size={14} /> Download All ({downloadUrls.length})
              </button>
            )}
            {duration > 0 && (
              <button
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent bg-zinc-900/50">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div
              onClick={addPlayer}
              className="flex flex-col bg-zinc-900/40 border-2 border-zinc-800 border-dashed hover:border-zinc-600 rounded-xl overflow-hidden shrink-0 w-64 items-center justify-center p-6 cursor-pointer transition text-zinc-500 hover:text-zinc-300 group min-h-[120px]"
            >
